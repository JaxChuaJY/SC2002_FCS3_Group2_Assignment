...
/**
 * Manages applications for BTO projects, including creating, retrieving, approving, and withdrawing applications.
 * Applications are stored in a map where the key is the project name and the value is a set of applications for that project.
 * This class interacts with {@link IProjectManager} and {@link IUserManager} to access project and user data.
 */
public class ApplicationManager implements IApplicationManager {
	...
	/**
     * Loads applications from a CSV file into the application list.
     * The CSV file is expected to have columns: projectName, nric, flatType, status.
     */
	@Override
     ...
     *
     * @param values the array of values from a CSV row (projectName, nric, flatType, status)
     ...
     * @throws IllegalArgumentException if the user is invalid, the project is invalid, or the user is an HDBOfficer managing the project
     */
	@Override
	public void createApplication(User user, Project project, FlatType flatType) {
		...
		if (applicant instanceof HDBOfficer officer
			    && officer.getProjects().contains(project)) {
     ...
     * @return a list of applications for the project, or an empty list if the project is null or has no applications
     */
	@Override
	...
	public void approveWithdrawal(User user) {
		...
		// Call this and ask for input etc.
		/*getApplicationsForProject(project).stream()
			.filter(a -> a.getStatus().equals(ApplicationStatus.WITHDRAW_REQUEST))
		...
		for (Set<Application> applications : applicationList.values()) {
			applications.removeIf(a -> 
				a.getApplicant().getNric().equals(user.getNric()));
		}
